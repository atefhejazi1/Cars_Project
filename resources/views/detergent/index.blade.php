...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  ...
  <thead class="text-center">
    <tr>
      ...
      <th class="border" scope="col"  style="width:300px">وصف المنظف</th>
  ...
  <tr>
       <td class=" border">{{ $detergents-> id }}</td>
       <td class=" border">{{ $detergents-> name }}</td>
       <td class=" border" >
       ...
       <td class=" border">{{ $detergents-> price }}</td>
        @if ($detergents-> featured == '1')
        <td class=" border fs-1">✅</td>
        @else
        <td class=" border fs-1">❌</td>         
        @endif       <td class=" border"><a href="{{ route('detergent.edit',$detergents->id) }}" class="btn btn-warning ">تعديل</a></td>
       <td class=" border">
        <form action="{{ route('detergent.destroy',$detergents->id) }}" method="post" style="margin-right: 10%;">
          @method('DELETE')
          @csrf

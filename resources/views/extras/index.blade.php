...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  ...
  <thead class="text-center">
    <tr>
      ...
      <th class="border" scope="col">وصف الاضافة</th>
  ...
  <tr>
       <td class="px-5 border">{{ $extrase-> id }}</td>
       <td class="px-5 border">{{ $extrase-> name }}</td>
       <td class="px-5 border">{{ $extrase-> description }}</td>
       <td class="px-5 border">
        <img src="{{ asset('images/extras/'.$extrase-> path_image)}}" width="70px" height="70px" alt="sameh"/>
       </td>
       <td class="px-5 border">{{ $extrase-> price }}</td>
       @if ($extrase-> featured == '1')
        <td class="px-5 border fs-1">✅</td>
        @else
        <td class="px-5 border fs-1">❌</td>         
        @endif
       <td class="px-5 border"><a href="{{ route('extras.edit',$extrase->id) }}" class="btn btn-warning ">تعديل</a></td>
       <td class="px-5 border">
        <form action="{{ route('extras.destroy',$extrase->id) }}" method="post">
          @method('DELETE')
          @csrf

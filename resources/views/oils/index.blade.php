...
@endif
  ...
  <thead class="text-center">
    <tr>
      ...
      <th class="border" scope="col"  style="width:300px">وصف الزيت</th>
  ...
  <tbody>
      @foreach ($oils as $oil)
  <tr>
       <td class="border">{{ $oil-> id }}</td>
       <td class="border">{{ $oil-> name }}</td>
       <td class="border">
       ...
       <td class="border">{{ $oil-> price }}</td>
       @if ($oil-> featured == '1')
        <td class="border fs-1">✅</td>
        @else
        <td class="border fs-1">❌</td>         
        @endif
       <td class="border"><a href="{{ route('oils.edit',$oil->id) }}" class="btn btn-warning ">تعديل</a></td>
       <td class="border">
